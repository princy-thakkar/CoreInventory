...
if(isset($_POST['add_receipt'])){
    ...
    if(!$supplier_id || empty($product_ids)){
        ...
        $stmt = $conn->prepare("INSERT INTO receipts(supplier_id) VALUES(?)");
        $stmt->execute([$supplier_id]);
        $receipt_id = $conn->lastInsertId();

        // Insert each product into receipt_items, update stock, log in stock_ledger
        for($i=0; $i<count($product_ids); $i++){
            $pid = intval($product_ids[$i]);
            $qty = intval($quantities[$i]);
            if($qty > 0){
                // 1️⃣ Insert into receipt_items
                $conn->prepare("INSERT INTO receipt_items(receipt_id, product_id, quantity) VALUES(?,?,?)")
                     ->execute([$receipt_id, $pid, $qty]);

                // 2️⃣ Update product stock
                $conn->prepare("UPDATE products SET stock = stock + ? WHERE id=?")
                     ->execute([$qty, $pid]);

                // 3️⃣ Log in stock_ledger
                $conn->prepare("
                    INSERT INTO stock_ledger (product_id, type, quantity, reference_id)
                    VALUES (?, 'Receipt', ?, ?)
                ")->execute([$pid, $qty, $receipt_id]);
            }
        }
...
<head>
<meta charset="UTF-8">
<title>Add Receipt - CoreInventory</title>
<style>
...
<button name="add_receipt">Add Receipt</button>
</form>
</div>
